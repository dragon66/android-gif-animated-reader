...
public class AnimatedGIFReader {
	...
	private int[] decodeLZW(InputStream is) throws Exception {
		int dimension = width*height;		
		int[] temp_ = new int[dimension];

		int min_code_size = is.read();// The length of the root
		LZWTreeDecoder decoder = new LZWTreeDecoder(is, min_code_size, rgbColorPalette);
		decoder.decode(temp_, 0, dimension);
		
		return temp_;
	}
   
	private int[] decodeLZWInterLaced(InputStream is) throws Exception	{
		int index = 0;
		int index2 = 0;
	 ...
	 * @return a Bitmap for the image or current frame in case of animated GIF
	 * @throws Exception
	 */
	protected Bitmap getFrameAsBitmap(InputStream is) throws Exception {
		// Read frame into a byte array
		int[] pixels = readFrame(is);
	...
	private int[] readFrame(InputStream is) throws Exception {
	...
	private boolean readGlobalScopeData(InputStream is) throws Exception {
		...
		if((flags&0x80) == 0x80) { // A global color map is present 
			...
			if(bgcolor < colorsUsed)
			   backgroundColor = globalColorPalette[bgcolor];
	   	}
	...
	private static class LZWTreeDecoder {
		 ...
		 */
		public LZWTreeDecoder(InputStream is, int min_code_size, int[] rgbColorPalette) {
		...
		public int decode(int[] pix, int offset, int len) throws Exception {
				...
				if(code == clearCode) {
				   ...
				   for( int j = i-1; j >= 0; j--, leftOver--, counter++ ) {
					   if ((offset >= pix.length)||(counter>=len))
				             break label;
					   pix[offset++] = rgbColorPalette[buf[j]&0xff];
			       }
			    }
	        } while(true);
